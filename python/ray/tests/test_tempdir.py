...
def unix_socket_create_path(name):
    ...
    return (
        os.path.join(ray._common.utils.get_default_temp_dir(), name) if unix else None
    )
...
def test_tempdir_created_successfully(delete_default_temp_dir, shutdown_only):
    temp_dir = os.path.join(ray._common.utils.get_default_temp_dir(), uuid.uuid4().hex)
...
def test_tempdir_long_path():
    if sys.platform != "win32":
        ...
        temp_dir = os.path.join(ray._common.utils.get_default_temp_dir(), "z" * maxlen)
